...
import {
  fetchCategories,
  fetchCategoriesById,
} from "store/reducers/categoryReduser";
...
const HeaderComponent: React.FC = () => {
  ...
  const handleMouseEnter = (key: string) => {
    ...
    if (!children[category]) {
      dispatch(
        fetchCategoriesById({ cancelToken: source.token, id: +key })
      ).then((res: any) => {
  ...
  return (
    <header className={classes.header}>
      ...
      <div className={classes.categories}>
        <Button
          onClick={!open ? showDrawer : onClose}
          className={classes.categories_btn}
          style={{ color: "black !important" }}
          type="primary"
          icon={!open ? <UnorderedListOutlined /> : <CloseOutlined />}
        >
          Каталог
        </Button>
        <ul>
      ...
      <div className={classes.test}>
        <div
          style={!open ? { height: "1px" } : { height: "100vh", opacity: "1" }}
